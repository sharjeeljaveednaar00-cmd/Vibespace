...
export default function App() {
  // ---------------- Auth / Onboarding ----------------
  const [authStep, setAuthStep] = useState('signup'); // signup -> profile -> verify -> done
        ...
        <div className="w-full max-w-sm space-y-6">
          ...
          {authStep === 'signup' && (
            <div className="space-y-3">
              ...
              {['Google', 'Facebook', 'TikTok'].map((m) => (
                <button key={m} onClick={() => { setSignupMethod(m); setAuthStep('profile'); }} className="w-full bg-slate-900 border border-slate-700 hover:border-purple-500/50 text-slate-100 font-bold text-sm py-3 rounded-xl">Continue with {m}</button>
              ))}
              <button onClick={() => { setSignupMethod('Email'); setAuthStep('profile'); }} className="w-full bg-slate-900 border border-slate-700 hover:border-purple-500/50 text-slate-100 font-bold text-sm py-3 rounded-xl">Continue with Email</button>
              <button onClick={() => { setSignupMethod('Phone'); setAuthStep('profile'); }} className="w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold text-sm py-3 rounded-xl">Continue with Phone Number</button>
          ...
          {authStep === 'profile' && (
            <div className="space-y-3">
              <p className="text-xs text-slate-400 text-center">Signed up with {signupMethod}. Now build your profile.</p>
              ...
              <button onClick={() => { setAuthStep('verify'); setVerifyProgress(0); }} disabled={!profileDraft.name} className="w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold text-sm py-3 rounded-xl disabled:opacity-40">Continue to Verification</button>
            </div>
          )}
        ...
        {activeTab === 'profile' && (
          <div className="p-6 max-w-4xl mx-auto w-full space-y-6">
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 flex items-center gap-6 flex-wrap">
              ...
              <div><h3 className="text-xl font-bold text-slate-100">{userProfile.name}</h3><p className="text-xs text-purple-400 mt-1">{userProfile.activeBadge}</p></div>
            </div>
